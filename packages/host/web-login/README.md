...
Mount `dsh-web-login` to give a Web deployment a real login page: unauthenticated browsers land on `/login`, sign in with a mainland phone number plus an SMS code (first verification creates the account) or a password, and leave with an account-bound session cookie; the page also redeems an optional invite code after login. Without an SMS provider, each code is logged server-side under an explicit demo marker. On top sits a member model: the first registered phone becomes the owner, single-use invites bind phones to AI-team roles, and `/auth/status` answers roles and ownership.
...
| `POST /auth/register` | Registers `phone` with `code` + `password` (8–64 chars, one letter and one digit), mints the cookie, and answers `{ok, redirect: '/'}`; 400 `weak-password`, 409 `phone-registered`. |
...
- [dsh-host-webserver](../webserver/README.md) — the route registry carrying the twelve HTTP endpoints.
...
Nothing. The package serves a human login flow over twelve HTTP routes (`/login`, `/auth/status`, `/auth/sms/send`, `/auth/sms/verify`, `/auth/register`, `/auth/password/login`, `/auth/password/reset`, `/auth/logout`, `/team/invites`, `/team/invites/redeem`, `/team/members`, `DELETE /team/members/:phone`) and an in-memory challenge store; it registers no tool, prompt section, message content, or event the model ever reads, and the logged-in identity stays server-side in the `web_login` domain and `ctx.loginSession`.
...
- **No real SMS provider.** Codes are logged server-side with an explicit `[演示]` marker; a deployment must not expose this route set to untrusted networks until a provider lands behind `issue`/`verify`.
