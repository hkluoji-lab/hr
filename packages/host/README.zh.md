---
description: "Web GUI Host 侧的包映射：HTTP 与 SPA 服务器、手机验证码登录表面、工作区目录选择实现、open-in-app 启动路由和插件清单投影。"
kind: "package-group"
---
...
`host/` 组提供 Web GUI 的普通 HTTP 服务器、服务已构建 Web 壳的 SPA dist 服务器、手机验证码登录表面、带原生／浏览／自适应组合包的工作区目录选择 seam、open-in-app 的应用探测与启动路由，以及只读的插件清单投影。这九个包都是产品包；浏览器传输位于 [`client/`](../client/README.zh.md)，组合应用是 [`apps/cli`](../../apps/cli/README.zh.md)，它启动 [`dsh-base` 组合包](../bundle/base/cordis.patch.yml) 来提供 `apps/web/` 下的 Web 应用。选择器后端可在共享 seam 后互相替换。
...
| [`web-login/`](web-login/README.zh.md) | 复用连接信任栅栏的手机号 + 短信验证码登录页与路由；`loginSession` 身份服务 | 提供 `ctx.loginSession` |
