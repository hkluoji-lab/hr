...
会话打开时，右栏会新增四个 builtin 页面型 tab，四者同时出现在 guide 页。**交付物**列出本会话成功的 `write`、`edit` 与变更类 `str_replace_editor` 调用产出的全部文件，按首次出现顺序去重；点击行通过 Sidebar 的资源地址打开文件。**进度**按会话存储顺序列出本会话的子任务成员，带「进行中/已结束」状态点；点击行打开对应子会话。**赏金额度**展示本部署的积分余额与近期发放明细。**AI 团队状态**统计在线、工作中与离线的成员数，再逐个列出成员及其状态点。无可展示内容的 tab 只渲染一条空状态、读取中或读取失败的提示。
