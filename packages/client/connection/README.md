...
Every Host RPC method and WebSocket stream requires one browser session; there is no method-specific loopback tier. Each process mints a random launch token. `dsh-web-app` prints and opens the ordinary root URL with `?token=...`; `frontend-static` delegates root and index requests to `ctx.connection.authorizeIndex`, which accepts that token only on `GET /`, writes an authority-bound signed cookie, and redirects to clean `/`. A missing, expired, malformed, or wrong-authority cookie returns 401 before RPC dispatch. Static assets remain public. The HTTP carrier accepts no query token outside the root exchange and no Authorization-header token. Setting Connection config `trustLoopback: true` relaxes only this index gate: any tokenless loopback request gets index.html without a token or cookie, so plain `http://127.0.0.1:<port>/` opens directly; the `?token=` exchange keeps precedence, so the printed URL still mints the cookie that `/api` requires; every other authority still returns 401 and the `/api` Host/Origin fence and cookie checks are unchanged.
