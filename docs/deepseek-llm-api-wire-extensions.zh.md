...
每个请求都会重新读取宿主树中的存活非分组 Loader 配置项；请求会话存在 standing agent-preset 树时，也会读取该树。相对与绝对模块使用距离自身最近的所属 manifest；裸包配置项使用激活自身的 Loader 解析基准。格式错误的包元数据，以及其包在磁盘上已无法解析的存活配置项（通常是包被移除而宿主未重启），都会带警告丢弃该行：本字段是遥测数据，请求仍会携带所有解析成功的行。
